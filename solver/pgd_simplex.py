"""
Projected Gradient Descent (PGD) on the simplex.
...
def PGD_simplex(problem, max_iters=400, step_size=1.0, armijo_c=1e-4, 
    ...
    for k in range(max_iters):
        ...
        if verbose and k % 50 == 0:
            print(f"[PGD] iter={k:4d} f={obj_val:.6e} ||grad||={grad_norm:.3e} time={current_time:.2f}s")
        ...
        while True:
            # Trial point after projection
            z = project_simplex(x - t * grad)
            fz = problem.f(z)
            
            # Armijo condition: sufficient decrease
            if fz <= fx - armijo_c * t * np.dot(grad, x - z) or t < 1e-16:
                break
