"""
Accelerated Projected Gradient Descent (APGD) with Nesterov acceleration for box constraints.
External baseline method for comparison with GRAVIDY–box.
"""
...
def APGD_box(problem, max_iters=200, step_size=None, x0=None, tol_grad=1e-8, verbose=False):
    """
    Accelerated Projected Gradient Descent with Nesterov acceleration for box constraints.
    ...
    Args:
        ...
        x0: Initial point (if None, uses midpoint of box)
    ...
    # Initialize at midpoint of box
    if x0 is None:
        x = 0.5 * (lo + hi)
    else:
        x = np.minimum(np.maximum(x0, lo), hi)
    ...
    if step_size is None:
        ...
        step_size = 1.0 / L
        if verbose:
            print(f"[APGD-box] Estimated Lipschitz constant: {L:.6f}, step size: {step_size:.6f}")
    ...
    for k in range(max_iters):
        ...
        if verbose and k % 50 == 0:
            print(f"[APGD-box] iter={k:4d} f={obj_val:.6e} ||grad||={grad_norm:.3e} time={current_time:.2f}s")
        
        # Check convergence
        if grad_norm <= tol_grad:
            break
        
        # Gradient step with projection (clipping)
        x_old = x.copy()
        x = np.minimum(np.maximum(y - step_size * grad, lo), hi)
